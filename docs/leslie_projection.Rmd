...
# The line below...
proj_year[1, ] <- fish_year0 #start with initial conditions 
#each row is a time step, that's why you dot product with proj_year
proj_year[2,] <- fish_leslie %*% proj_year[1,]

# You find the population size at the next time step by doing the dot product 
# The for loop below...
for (i in 2:length(time_yr)) {
  proj_year[i,] <- fish_leslie %*% proj_year[i-1,] #individuals previous year
}
